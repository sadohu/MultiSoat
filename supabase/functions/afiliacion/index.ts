  ...
  try {
    ...
    switch (req.method) {
      ...
      case "POST": {
        ...
        const { data: puntoVenta, error: puntoVentaError } =
        ...
        if (newAfiliacion.id_distribuidor) {
          const { data: distribuidor, error: distribuidorError } =
            await supabaseClient
              .from("distribuidor")
              .select("id")
        ...
        if (existingAfiliacion) {
          return new Response(
            JSON.stringify({
              ...
              status: 400,
              error:
                "Ya existe una afiliación entre este proveedor y punto de venta",
            }),
            {
              status: 400,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      case "PUT": {
        ...
        if (
          updateData.id_proveedor &&
          updateData.id_proveedor !== existingAfiliacion.id_proveedor
        ) {
          const { data: proveedor, error: proveedorError } =
            await supabaseClient
              .from("proveedor")
              .select("id")
        ...
        if (
          updateData.id_punto_venta &&
          updateData.id_punto_venta !== existingAfiliacion.id_punto_venta
        ) {
          const { data: puntoVenta, error: puntoVentaError } =
            await supabaseClient
              .from("punto_venta")
